...
with h5py.File(train_h5F_addr,"r") as train_file:
    with tf.Session() as sess:
        sess.run(init_op)
        #saver_full.restore(sess, "/media/kenny/Data/trained_models/multi_dense_guide_resnet/noise_free/l1loss/8x/full_model1/8x_nf_full_model.ckpt-22")
        for epo in range(epo_range):
            ...
            for ind in range(batch_total):
                ...
                if (ind+1)%7656==0:
                    save_path=saver_full.save(sess,"/media/kenny/Data/trained_models/multi_dense_guide_resnet/noise_free/l1loss/8x/full_model1/8x_nf_full_model.ckpt",global_step=model_ind)
